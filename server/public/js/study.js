...
(async () => {
  try {
    ...
    document.getElementById('content').innerHTML =
      ...
      '<div class="card-grid">' +
        '<div class="card"><div class="label">Sessions</div><div class="value">' + aggregate.sessions + '</div></div>' +
        ...
        '<div class="card"><div class="label">Fixations</div><div class="value">' + aggregate.fixations + '</div></div>' +
        '<div class="card"><div class="label">Engagement</div><div class="value">' + (aggregate.engagement ? aggregate.engagement.score : 0) + '</div></div>' +
        '<div class="card"><div class="label">Participants</div><div class="value">' + participants.length + '</div></div>' +
      '</div>' +
      '<h3 style="margin:24px 0 12px">Groups</h3>' +
      '<div class="card-grid">' +
        Object.entries(aggregate.groups || {}).map(function(entry) {
          return '<div class="card"><div class="label">' + esc(entry[0]) + '</div><div class="value">' + entry[1].participants + ' / ' + entry[1].sessions + ' sessions</div></div>';
        }).join('') +
      '</div>';
  } catch (err) {
