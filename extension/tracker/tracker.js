...
const state = {
  ...
  currentPosition: null,    // MediaPipe face landmarks
  currentEyeRect: null,     // Eye crop rectangle
  samples: { train: [], val: [] },
  sampleCount: 0,
  model: null,
  bestModel: null,
...
function extractEyeRegion(landmarks) {
  // Get bounding box around both eyes (landmarks 33-133 left, 362-263 right)
...
async function initWebcam() {
  try {
    ...
    // Start face mesh processing loop
    startProcessingLoop();
  } catch (err) {
    console.error('Webcam error:', err);
    setStatus('webcam', 'error');
    setStatusText('Webcam access denied');
  }
}

function startProcessingLoop() {
  async function processFrame() {
    if (state.webcamReady && faceMesh && webcamEl.readyState >= 2) {
...
function captureExample(targetX, targetY) {
  ...
  const target = [targetX, targetY]; // Normalized 0-1
...
function prepareTensors(samples) {
  ...
  for (const s of samples) {
    // Reshape grayscale array to [H, W, 1]
    const img = [];
    for (let y = 0; y < EYE_CANVAS_H; y++) {
      const row = [];
      for (let x = 0; x < EYE_CANVAS_W; x++) {
        row.push([s.image[y * EYE_CANVAS_W + x]]);
      }
      img.push(row);
    }
    images.push(img);
    metas.push(s.meta);
    // Convert target from 0-1 to -1..1 for tanh output
...
async function trainModel() {
  ...
  // Ensure validation set has data
  if (state.samples.val.length < 2) {
    // Move some training data to validation
    while (state.samples.val.length < 2 && state.samples.train.length > 5) {
      state.samples.val.push(state.samples.train.pop());
    }
  }

  const trainData = prepareTensors(state.samples.train);
  const valData = prepareTensors(state.samples.val);
  ...
  try {
    await state.model.fit([trainData.images, trainData.metas], trainData.targets, {
      epochs,
      batchSize: Math.min(32, state.samples.train.length),
      ...
      callbacks: {
        onEpochEnd: (epoch, logs) => {
          ...
          if (logs.val_loss && logs.val_loss < state.bestValLoss) {
            ...
            // Clone weights for best model
            if (state.bestModel) state.bestModel.dispose();
            state.bestModel = null; // Will use current model if it's the best
          }
        }
      }
    });
...
function getPrediction() {
  ...
  return tf.tidy(() => {
    ...
    // Convert from -1..1 (tanh) back to 0..1
...
function onCalibrationClick(e) {
  ...
  // Capture multiple samples at this point
  let captured = 0;
  for (let i = 0; i < 3; i++) {
...
function toggleAutoCollect() {
  ...
  // Track mouse/touch position for auto-collection
  ...
  state._autoMoveHandler = moveHandler;
}
...
function startTracking() {
  ...
  // Send predictions to background script
  state.trackingInterval = setInterval(() => {
    ...
    if (pred) {
      chrome.runtime.sendMessage({
        ...
        y: pred.y,
        timestamp: Date.now(),
        confidence: state.faceDetected ? 1 : 0,
      });
    }
  }, 50); // 20 Hz gaze updates
...
async function saveModel() {
  ...
  try {
    await state.model.save('indexeddb://eyed-model');
    // Also save dataset
...
async function loadModel() {
  try {
    state.model = await tf.loadLayersModel('indexeddb://eyed-model');
    state.model.compile({ optimizer: tf.train.adam(0.001), loss: 'meanSquaredError' });

    // Load dataset
    const result = await chrome.storage.local.get(['eyedDataset', 'eyedSampleCount']);
    if (result.eyedDataset) {
      state.samples = JSON.parse(result.eyedDataset);
  ...
  input.onchange = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        state.sampleCount = data.sampleCount;
        updateProgress();
        setStatusText('Data imported');
      } catch (err) {
        setStatusText('Import failed — invalid file');
      }
    };
...
function resetAll() {
  ...
  tf.io.removeModel('indexeddb://eyed-model').catch(() => {});
}
...
function bindEvents() {
  ...
  // Calibration target click/touch
